...
export class DPS extends Character {
    ...
    updateTarget() {
        ...
        this.targetEnemy = this.parent.enemies.find(enemy => enemy.health.current > 0) || null
    }
    ...
    attackTarget() {
        ...
        const actualDamage = this.targetEnemy.takeDamage(damage)
        this.lastAttackTime = this.parent.elapsedTime

        // Log the attack
        log(`dps: ${this.constructor.name} dealt ${actualDamage} damage to ${this.targetEnemy.name}`)

        // Create floating combat text
        const fct = html`<floating-combat-text class="damage">-${actualDamage}</floating-combat-text>`.toDOM()
